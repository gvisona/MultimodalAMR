...
DATASETS = ['A', 'B', 'C', 'D']
TRAINING_SETUPS = list(itertools.product(DATASETS, ["random"], np.arange(5)))
...
def main(args):
    ...
    dset_idx = DATASETS.index(args.driams_dataset)
    spectra_matrix = np.load(args.spectra_matrices[dset_idx]).astype(float)
    ...
    dsplit = DataSplitter(driams_long_table.copy(), dataset=args.driams_dataset)
    samples_list = sorted(dsplit.long_table["sample_id"].unique())

    # Split selection for the different experiments.
    if args.split_type == "random":
        train_df, val_df, test_df = dsplit.random_train_val_test_split(val_size=0.1, test_size=0.1, random_state=args.seed)
        val_df = pd.concat([val_df, test_df])
    ...
    datasets_for_transfer = [ds for ds in DATASETS if ds!=args.driams_dataset]
    for target_dataset in datasets_for_transfer:
        print(f"Processing dataset {target_dataset}")
        target_dsplit = DataSplitter(driams_long_table.copy(), dataset=target_dataset)
        target_df = target_dsplit.long_table.copy()
        
        target_samples_list = sorted(target_df["sample_id"].unique())
        target_dset_idx = DATASETS.index(target_dataset)
        target_spectra_matrix = np.load(args.spectra_matrices[target_dset_idx]).astype(float)
        target_dset = DrugResistanceDataset_Fingerprints(target_df, target_spectra_matrix, drugs_df, target_samples_list, fingerprint_class=config["fingerprint_class"])
        target_loader = DataLoader(
            target_dset, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers)


        experiment.test_predictions = []
        test_results = trainer.test(ckpt_path="best", dataloaders=target_loader)
        with open(join(results_folder, "test_metrics_{}.json".format(seed)), "w") as f:
            json.dump(test_results[0], f, indent=2)
            
            experiment.model.eval()
            target_df["Predictions"] = experiment.test_predictions
            target_df.to_csv(join(results_folder, f"target_DRIAMS-{target_dataset}_seed{seed}.csv"), index=False)
...
if __name__=="__main__":
    ...
    parser.add_argument("--experiment_name", type=str, default="DirectTransfer")
    ...
    parser.add_argument("--spectra_matrices", nargs='+', default=[])
